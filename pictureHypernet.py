...
# This file attempts to train the hypernet to reconstruct the image.
...
class HyperNet2(nn.Module):
    def __init__(self, out_size):
        ...
        self.cnn_layers = nn.Sequential(
            ...
            nn.Conv2d(20, 60, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(60),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2),
            # Defining another 2D convolution layer
            ...
            nn.MaxPool2d(kernel_size=2, stride=2),
            # Defining another 2D convolution layer
            nn.Conv2d(20, 6, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
        )

        self.linear_layers = nn.Sequential(
            nn.Linear(96, out_size)
        )
    ...
    def forward(self, x):
        ...
        for k in range(BATCH_SIZE):
            ...
            locs_vec = torch.FloatTensor([[i / H, j / W] for i in range(H) for j in range(W)])
...
class SimpleNet(nn.Module):
    def __init__(self, weights):
        ...
        layers = [nn.Linear(2, 8), nn.Linear(8, 32), nn.Linear(32, 32), nn.Linear(32, 8), nn.Linear(8, 3)]
        offset = 0
        for layer in layers:
...
for epoch in tqdm(range(EPOCHS)):
    ...
    for i in tqdm(range(num_batchs)):
        ...
        optimizer.zero_grad()
        x_batch = torch.reshape(x_batch, [BATCH_SIZE, 3, H, W])
        outputs = net(x_batch)
